...
async function main () {
  ...
  const nft: MyNFT = await NFT.deploy(_name, _symbol)
  const tokenAddress = nft.target
  ...
  await market.connect(account1).buy(tokenAddress, 1, { value: auctionPrice })
...
async function parseItems (items: any, nft: BadgeToken) {
  const parsed = await Promise.all(items.map(async (item: any) => {
...
function parseNFT (item: any) {
  const data = atob(item.tokenUri.slice(29))
  const itemInfo = JSON.parse(data)
  const svg = atob(itemInfo.image.slice(26))
  return (
    {
      name: itemInfo.name,
